...
# Описание поведенческой модели стека
...
- Стек может изменяться только при **CLK = 1** или **RESET = 1**. Это обрабатывается в строке:

  ```verilog
  always @(posedge CLK, RESET)

## Логика работы
- Если RESET равен 1, программа заново инициализируется.
- Если CLK равен 1, программа проверяет значение COMMAND с помощью конструкции if и выполняет соответствующую  операцию, изменяя массив stack и значение curr_idx.

## Замечания
Обратите внимание, что когда RESET включается при включенном CLK, а затем выключается, команда все равно будет срабатывать. Это происходит потому, что программа войдет в блок always, а затем уже в блоки if.
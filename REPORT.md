...
Это описывается при помощи блоков if
   ...
   Если COMMAND == 2'b00 - ничего не происходит
   Если COMMAND == 2'b01 - операция push, но если она уже была произведена на данном такте, то происходит изменение верзней ячейки стека на IO_DATA_in ( в реализации мы будет использовать флга is_push)
   Если COMMAND == 2'b10 - операция pop, но если она уже была произведена на данном такте, то ничего не происходит ( в реализации мы будет использовать флга is_pop)
   Если COMMAND == 2'b11 - операция get. возвращает значение ячейки памяти по текущей curr_idx и index
